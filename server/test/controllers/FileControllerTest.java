...
public class FileControllerTest extends WithMockedProfiles {
  ...
  // Applicant ACL is additive to the file key "acl", so we can only test for the additive cases and
  // not the ones that would not allow access. Those tests are done at the StoredFileAcls test
  // level.
  @Test
  public void show_applicantAcl_differentApplicantAcled_success() {
    ...
    // Add the Applicant Id Acl.
    createStoredFileWithApplicantAccess(anothersFileKey, applicant.id);
    result = controller.show(request, applicant.id, anothersFileKey).toCompletableFuture().join();
    assertThat(result.status()).isEqualTo(SEE_OTHER);
  }
  ...
  @Test
  public void adminShow_programAdmin_differentProgram_unauthorized() {
  ...
  @Test
  public void adminShow_globalAdminWithPrivileges_differentFileKey_notFound() {
  ...
  @Test
  public void adminShow_globalAdmin_unauthorized() {
    ProgramModel program = ProgramBuilder.newDraftProgram().build();
    createProgramAdminWithMockedProfile(program);
    createGlobalAdminWithMockedProfile();
    String fileKey = creatFakeFileKey(1L, program.id);
  ...
  @Test
  public void adminShow_globalAdminWithPrivileges_success() {
    when(mockSettingsManifest.getAllowCiviformAdminAccessPrograms(request)).thenReturn(true);
    ProgramModel program = ProgramBuilder.newDraftProgram().build();
    createGlobalAdminWithMockedProfile();
    String fileKey = creatFakeFileKey(1L, program.id);
  ...
  @Test
  public void acledAdminShow_programAdmin_differentProgram_unauthorized() {
    ProgramModel programOne = ProgramBuilder.newDraftProgram("one").build();
    ProgramModel programTwo = ProgramBuilder.newDraftProgram("two").build();
    createProgramAdminWithMockedProfile(programOne);
  ...
  @Test
  public void acledAdminShow_globalAdminWithPrivileges_differentFileKey_notFound() {
    when(mockSettingsManifest.getAllowCiviformAdminAccessPrograms(request)).thenReturn(true);
    createGlobalAdminWithMockedProfile();
    ProgramModel program = ProgramBuilder.newDraftProgram().build();
    createStoredFileWithProgramAccess(creatFakeFileKey(1L, program.id), program);
  ...
  @Test
  public void acledAdminShow_globalAdminWithoutPrivileges_unauthorized() {
    when(mockSettingsManifest.getAllowCiviformAdminAccessPrograms(request)).thenReturn(false);
    ProgramModel program = ProgramBuilder.newDraftProgram().build();
    createGlobalAdminWithMockedProfile();
    String fileKey = creatFakeFileKey(1L, program.id);
  ...
  @Test
  public void acledAdminShow_globalAdminWithPrivileges_success() {
    when(mockSettingsManifest.getAllowCiviformAdminAccessPrograms(request)).thenReturn(true);
    ProgramModel program = ProgramBuilder.newDraftProgram().build();
    createGlobalAdminWithMockedProfile();
    String fileKey = creatFakeFileKey(1L, program.id);
  ...
  private void createStoredFileWithApplicantAccess(String fileKey, long applicantId) {
    var file = new StoredFileModel().setName(fileKey);
    file.getAcls().addApplicantToReaders(applicantId);
    file.save();
  }
}
