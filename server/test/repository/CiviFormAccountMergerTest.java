...
public class CiviFormAccountMergerTest extends ResetPostgres {
  ...
  public void mergeApplicants_bothActive_oneNewer(Newer newerApplication) {
    ...
    // Create Actives for both as the main test.  Add Drafts to verify that
    // secondarily. Drafts are not so important that we need to test every
    // combination of their existence alongside the Actives.
    ...
    if (newerApplication.equals(Newer.CIVIFORM)) {
      assertThat(cfApps)
          .extracting(a -> a.id)
          .containsExactlyInAnyOrder(guestActive.id, cfActive.id, cfDraft.id);
      ...
      assertThat(cfApps)
          .extracting(a -> a.id)
          .containsExactlyInAnyOrder(cfActive.id, guestActive.id, guestDraft.id);
