...
// questions about nesting:
// - if the inner transaction fails, should the outer transaction also roll back?
// - if the outer transaction fails, should the inner transaction roll back?

// ok I get the same error if it's a scoped transaction.
// if it's not a scoped transaction I get an "we're already in a transaction" error.
// what if I use a try-with-resources instead? same error.

// so transactions...
// does creating a new version and creating a question need to be in the same transaction?
// someone could publish right as you're creating the version, so yes.

// flow:
// - check conflicts
// - get draft version if it exists
// - inner transaction: create draft version if we need to
// -    is it ok if this sees a different application state then the outer transaction?
// - add the version to the question
