...
/**
 * Renders the fragments in {@code admin/questions/fragments/LegacySvgFragments.html} with
 * thymeleaf-testing and compares the result against the expected markup.
 *
 * <p>Each case is a {@code .thtest} file under {@code
 * test/resources/thymeleaf/admin/questions/legacySvgFragments/}: it declares the fragment call and
 * the markup the fragment is expected to produce. Each icon the migrated pages use is exercised
 * with every sizing-class combination the legacy j2html admin views pass to {@code Icons.svg} for
 * it, so migrated pages can reuse these fragments beyond the question pages without visual drift.
 */
public class LegacySvgFragmentsTest {

  private static final String DIR = "admin/questions/legacySvgFragments/";
