...
public final class ProgramMigrationServiceTest extends ResetPostgres {
  ...
  public void serializeThenDeserialize_dropdownWithScores_roundTrips() {
    ...
    assertThat(question.getOptions().stream().map(QuestionOption::score))
        .containsExactly(Optional.of(10), Optional.empty());
  }
  ...
  @Test
  public void deserialize_floatScore_returnsError() {
    ErrorAnd<ProgramMigrationWrapper, String> result =
        service.deserialize(wrapperJsonWithScoreLiteral("1.5"));

    assertThat(result.isError()).isTrue();
    assertThat(result.getErrors().stream().findFirst().get())
        .contains("Option score '1.5' on option 'opt1' must be a whole number");
  }
  ...
  public void deserialize_stringScore_returnsError() {
    ...
    assertThat(result.isError()).isTrue();
    assertThat(result.getErrors().stream().findFirst().get()).contains("must be a whole number");
  }

  @Test
  public void deserialize_outOfRangeScore_returnsError() {
    // 2^31 is one more than Integer.MAX_VALUE.
    ErrorAnd<ProgramMigrationWrapper, String> result =
        service.deserialize(wrapperJsonWithScoreLiteral("2147483648"));
  ...
  @Test
  public void deserialize_exponentScore_returnsError() {
    ErrorAnd<ProgramMigrationWrapper, String> result =
        service.deserialize(wrapperJsonWithScoreLiteral("1e3"));
