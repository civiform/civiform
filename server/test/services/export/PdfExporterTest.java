...
public class PdfExporterTest extends AbstractExporterTest {
  ...
  private void addScoreMetadataToApplicationOne(
      long total, long dropdownScore, List<Long> checkboxScores) {
  ...
  public void exportApplication_withScores_asAdmin_rendersTotalAndPerAnswerScores()
    ...
    checkboxScores.add(2L);
    checkboxScores.add(null);
    addScoreMetadataToApplicationOne(/* total= */ 7L, /* dropdownScore= */ 5L, checkboxScores);
    PdfExporter exporter = instanceOf(PdfExporter.class);
    ...
    assertThat(text).contains("Total score: 7");
    assertThat(text).contains("Score: 5");
    assertThat(text).contains("(Score: 2)");
  }
  ...
  public void exportApplication_checkboxScoreLengthMismatch_rendersAnswerWithoutScores()
    ...
    List<Long> mismatchedScores = new ArrayList<>();
    mismatchedScores.add(2L);
    addScoreMetadataToApplicationOne(/* total= */ 7L, /* dropdownScore= */ 5L, mismatchedScores);
    PdfExporter exporter = instanceOf(PdfExporter.class);
    ...
    assertThat(text).contains("Total score: 7");
    assertThat(text).contains("Score: 5");
    assertThat(text).doesNotContain("(Score: 2)");
  }
