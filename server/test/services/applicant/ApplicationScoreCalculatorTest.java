...
public class ApplicationScoreCalculatorTest extends ResetPostgres {
  ...
  private static ImmutableList<QuestionOption> scoredOptions() {
    return ImmutableList.of(
        option(1L, 0L, "ten", Optional.of(10)),
        option(2L, 1L, "unscored", Optional.empty()),
        option(3L, 2L, "minus_four", Optional.of(-4)),
        option(4L, 3L, "zero", Optional.of(0)));
  }

  private static QuestionOption option(
      long id, long displayOrder, String adminName, Optional<Integer> score) {
  ...
  public void enrich_singleSelect_scoredOption_writesScoreAndTotal() {
    ...
    assertThat(snapshot.readLong(ApplicationScoreMetadata.scorePath(path))).hasValue(10L);
    assertThat(snapshot.readLong(ApplicationScoreMetadata.totalScorePath())).hasValue(10L);
  }
  ...
  public void enrich_singleSelect_negativeScore_contributesNegative() {
    ...
    assertThat(snapshot.readLong(ApplicationScoreMetadata.scorePath(path))).hasValue(-4L);
    assertThat(snapshot.readLong(ApplicationScoreMetadata.totalScorePath())).hasValue(-4L);
  }
  ...
  public void enrich_singleSelect_unscoredOption_writesNoKeyButZeroTotal() {
    ...
    assertThat(snapshot.readLong(ApplicationScoreMetadata.totalScorePath())).hasValue(0L);
  }
  ...
  public void enrich_checkbox_mixedSelections_writesAlignedNullableArray() {
    ...
    assertThat(snapshot.readNullableLongList(ApplicationScoreMetadata.scoresPath(path)))
        .hasValue(Arrays.asList(-4L, null, null, null, 10L));
    // Only the first occurrence of the duplicated id contributes.
    assertThat(snapshot.readLong(ApplicationScoreMetadata.totalScorePath())).hasValue(6L);
  ...
  public void enrich_checkbox_zeroScoredOption_contributesExplicitZero() {
    ...
    assertThat(snapshot.readNullableLongList(ApplicationScoreMetadata.scoresPath(path)))
        .hasValue(Arrays.asList(0L));
    assertThat(snapshot.readLong(ApplicationScoreMetadata.totalScorePath())).hasValue(0L);
  }

  @Test
  public void enrich_extremeScores_totalIsSixtyFourBit() {
    QuestionDefinition checkbox =
        saveMultiOptionQuestion(
            "extreme checkbox",
            MultiOptionQuestionType.CHECKBOX,
            ImmutableList.of(
                option(1L, 0L, "max_one", Optional.of(Integer.MAX_VALUE)),
                option(2L, 1L, "max_two", Optional.of(Integer.MAX_VALUE))));
    ProgramDefinition program = programWith(checkbox);
    Path path = questionPath(checkbox);
    ...
    // 2 * Integer.MAX_VALUE overflows an int but not a long.
    assertThat(snapshot.readLong(ApplicationScoreMetadata.totalScorePath()))
        .hasValue(2L * Integer.MAX_VALUE);
  }
  ...
  public void enrich_yesNoQuestion_neverScores() {
    ...
    QuestionDefinition yesNo =
        saveMultiOptionQuestion(
            ...
            ImmutableList.of(
                option(1L, 0L, "yes", Optional.of(100)), option(0L, 1L, "no", Optional.of(50))));
  ...
  public void enrich_repeatedQuestion_scoresEachInstance() {
    ...
    assertThat(snapshot.readLong(ApplicationScoreMetadata.scorePath(firstInstance))).hasValue(10L);
    assertThat(snapshot.readLong(ApplicationScoreMetadata.scorePath(secondInstance)))
        .hasValue(-4L);
    assertThat(snapshot.readLong(ApplicationScoreMetadata.totalScorePath())).hasValue(6L);
  }
