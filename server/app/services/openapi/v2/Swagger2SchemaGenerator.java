...
public class Swagger2SchemaGenerator extends AbstractOpenApiSchemaGenerator
  ...
  private ObjectProperty buildResultItemProperty(ProgramDefinition programDefinition)
    ...
    if (openApiSchemaSettings.includeScores()) {
      // Null when the application predates scoring or scoring wasn't applied; 64-bit.
      resultItem.property("total_score", new LongProperty().vendorExtension("x-nullable", true));
    }
    return resultItem;
  }
      ...
      if (questionDefinition.getQuestionType() != QuestionType.ENUMERATOR) {
        ...
        if (openApiSchemaSettings.includeScores()
          ...
          if (questionDefinition.getQuestionType() == QuestionType.CHECKBOX) {
            containerDefinition.property(
                "scores",
                new ArrayProperty(new IntegerProperty().vendorExtension("x-nullable", true))
                    .vendorExtension("x-nullable", true));
          } else {
            containerDefinition.property(
                "score", new IntegerProperty().vendorExtension("x-nullable", true));
          }
        }
      } else {
