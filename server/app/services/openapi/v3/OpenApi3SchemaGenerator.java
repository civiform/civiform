...
public class OpenApi3SchemaGenerator extends AbstractOpenApiSchemaGenerator
  ...
  private ObjectSchema buildResultItemSchema(ProgramDefinition programDefinition)
    ...
    if (openApiSchemaSettings.includeScores()) {
      ...
      resultItem.addProperty(
          "total_score", new IntegerSchema().format("int64").nullable(true));
    }
    return resultItem;
  }
        ...
        if (openApiSchemaSettings.includeScores()
          ...
          if (questionDefinition.getQuestionType() == QuestionType.CHECKBOX) {
            containerDefinition.addProperty(
                "scores",
                new ArraySchema()
                    .items(new IntegerSchema().format("int32").nullable(true))
                    .nullable(true));
          } else {
            containerDefinition.addProperty(
                "score", new IntegerSchema().format("int32").nullable(true));
          }
        }
      } else {
