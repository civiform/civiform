...
public final class MapQuestion extends AbstractQuestion {
  ...
  /**
   * In a MAP question, filters include any setting that has {@link MapSettingType#LOCATION_FILTER_GEO_JSON_KEY} type. The
  ...
  public String getNameValue() {
    return getSettingValue(applicantQuestion.getApplicant().getApplicantData().preferredLocale(), MapSettingType.LOCATION_NAME_GEO_JSON_KEY);
  }

  public String getAddressValue() {
    return getSettingValue(applicantQuestion.getApplicant().getApplicantData().preferredLocale(), MapSettingType.LOCATION_ADDRESS_GEO_JSON_KEY);
  }

  public String getDetailsUrlValue() {
    return getSettingValue(applicantQuestion.getApplicant().getApplicantData().preferredLocale(), MapSettingType.LOCATION_DETAILS_URL_GEO_JSON_KEY);
  }

  private String getSettingValue(Locale locale, MapSettingType settingType) {
    Optional<LocalizedQuestionSetting> localizedQuestionSetting = getSettings(locale).stream()
      .filter(setting -> setting.settingType() == settingType).findFirst();
