...
public final class ApplicationScoreCalculator {
  ...
  public void enrich(
    ...
    for (ApplicantQuestion question : questions) {
      ...
      ImmutableMap<Long, Integer> scoresByOptionId =
          optionScoresById((MultiOptionQuestionDefinition) question.getQuestionDefinition());
      if (question.getType() == QuestionType.CHECKBOX) {
        total += enrichCheckboxQuestion(snapshot, contextualizedPath, scoresByOptionId);
      } else {
        total += enrichSingleSelectQuestion(snapshot, contextualizedPath, scoresByOptionId);
      }
    }

    snapshot.putLong(ApplicationScoreMetadata.totalScorePath(), total);
  }

  private static ImmutableMap<Long, Integer> optionScoresById(
   ...
   * absence.
   */
  private static long enrichSingleSelectQuestion(
      ApplicantData snapshot, Path contextualizedPath, ImmutableMap<Long, Integer> scoresById) {
    ...
    long score = scoresById.get(selectedOptionId.get());
    snapshot.putLong(ApplicationScoreMetadata.scorePath(contextualizedPath), score);
    return score;
  }
   ...
   */
  private static long enrichCheckboxQuestion(
      ApplicantData snapshot, Path contextualizedPath, ImmutableMap<Long, Integer> scoresById) {
    ...
    for (Long selectedOptionId : maybeSelections.get()) {
      Integer score = scoresById.get(selectedOptionId);
