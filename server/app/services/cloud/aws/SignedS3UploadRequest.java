...
public abstract class SignedS3UploadRequest implements StorageUploadRequest {
  ...
  public String successActionRedirect() {
    if (fullSuccessActionRedirect().isPresent()) {
      return fullSuccessActionRedirect().get();
    } else if (successActionRedirectPrefix().isPresent()) {
      return successActionRedirectPrefix().get();
    } else {
      throw new IllegalArgumentException("Must set f or s");
    }
  }

  /** Redirect URL when the upload is successful. */
  public abstract Optional<String> fullSuccessActionRedirect();

  public abstract Optional<String> successActionRedirectPrefix();
  ...
  public abstract static class Builder {
    ...
    public abstract Builder setFullSuccessActionRedirect(String fullSuccessActionRedirect);

    /** Get successActionRedirect. This is used to build the policy. */
    abstract Optional<String> fullSuccessActionRedirect();

    /**
     * Sets the prefix for the redirect URL that the user will be redirected to once their file has
     * been successfully uploaded to AWS.
     *
     * <p>Context: This redirect URL is embedded in the file upload form in two places: 1) In the
     * `<input name="success_action_redirect">` element. (See {@link
     * views.fileupload.AwsFileUploadViewStrategy#additionalFileUploadFormInputs}.) 2) In the
     * `<input name="Policy">` element as part of the encoded policy string. (See {@link
     * #buildPolicy}.) The redirect URL needs to match these two places, otherwise AWS will throw a
     * policy error (see https://github.com/civiform/civiform/issues/6737). However, to fix
     * https://github.com/civiform/civiform/issues/6450, the redirect URL needs to be different
     * depending on which button the applicant clicked. So, instead of our upload request requiring
     * the "success_action_redirect" and the "Policy" to exactly match, we just require that they
     * have the same prefix as specified by {@code successActionRedirectPrefix}. This prefix must be
     * **as specific as possible**. And if the successActionRedirect will always have the same
     * value, {@code successActionRedirectPrefix} should be
     *
     * <p>assets/javascripts/file_upload.ts may modify the last parameter of the
     * success_action_redirect URL to specify where the user should be taken after the file has been
     * successfully uploaded. Using StartsWith here ensures that the last parameter can be changed
     * without problems, while still enforcing strong security on the redirect URL. See
     * https://github.com/civiform/civiform/issues/6737 for more details.
     */
    public abstract Builder setSuccessActionRedirectPrefix(String successActionRedirectPrefix);

    /** Get successActionRedirect. This is used to build the policy. */
    abstract Optional<String> successActionRedirectPrefix();
    ...
    Builder buildPolicy() {
      ...
      if (fullSuccessActionRedirect().isPresent()) {
        builder.setFullSuccessActionRedirect(fullSuccessActionRedirect().get());
      } else if (successActionRedirectPrefix().isPresent()) {
        builder.setSuccessActionRedirectPrefix(successActionRedirectPrefix().get());
      } else {
        throw new IllegalArgumentException(
            "Either fullSuccessActionRedirect or successActionRedirectPrefix must be set");
      }
  ...
  abstract static class UploadPolicy {
    ...
    abstract static class Builder {
      ...
      Builder setFullSuccessActionRedirect(String fullSuccessActionRedirect) {
        conditionsBuilder().add(SuccessActionRedirect.create(fullSuccessActionRedirect));
        return this;
      }

      Builder setSuccessActionRedirectPrefix(String successActionRedirectPrefix) {
        conditionsBuilder()
            .add(StartsWith.create("success_action_redirect", successActionRedirectPrefix));
        return this;
      }
