...
public final class PdfExporterService {
  ...
  public PdfExporter.InMemoryPdf generatePdf(Long applicationId, ProgramDefinition program) {
    Optional<Application> applicationMaybe = applicationService.getApplication(applicationId, program);
    if (applicationMaybe.isEmpty()) {
      throw new NoSuchElementException(String.format("Application %d does not exist.", applicationId));
    }
