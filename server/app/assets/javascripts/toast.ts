...
const MESSAGE_CLASS = 'cf-toast'
const MESSAGE_DATA_CLASS = 'cf-toast-data'
const INFO_SVG_PATH =
...
const SUCCESS_SVG_PATH = 'M5 13l4 4L19 7'
...
export function init() {
  ...
  toastContainer.setAttribute('id', CONTAINER_ID)
...
export function showToastMessage(message: ToastMessage) {
  ...
  toastMessage.classList.add(MESSAGE_CLASS)
  ...
  toastMessage.appendChild(getToastIcon(message.type))
  ...
  const toastContainer = document.getElementById(CONTAINER_ID)
  if (toastContainer) {
    ...
    if (message.duration > 0) {
      setTimeout(() => {
        dismissToast(message.id, /* dismissClicked = */ false)
      }, message.duration)
    }
  }
}

function getToastIcon(type: string): Element {
  ...
  if (type === 'alert') {
    svgPath.setAttribute('d', INFO_SVG_PATH)
  } else if (type === 'error') {
    svgPath.setAttribute('d', ERROR_SVG_PATH)
  } else if (type === 'success') {
    svg.setAttribute('fill', 'none')
    svg.setAttribute('stroke', 'currentColor')
    svg.setAttribute('stroke-width', '2')
    svgPath.setAttribute('d', SUCCESS_SVG_PATH)
  } else if (type === 'warning') {
    svgPath.setAttribute('d', WARNING_SVG_PATH)
  }
  return svgContainer
}

/** If a toast message is present, create it and add it to the container. */
function maybeShowToasts() {
  const messages = Array.from(
    document.querySelectorAll('.' + MESSAGE_DATA_CLASS),
  )
...
function dismissClicked(event: Event) {
  ...
  const toast = target.closest('.' + MESSAGE_CLASS)
