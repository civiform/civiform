...
/** Represents a sorting option for a list of Trusted Intermediaries. */
public enum TrustedIntermediarySortOption {
    LAST_MODIFIED(
    ...
    // ascending or
    // descending order. The full value will be in the format
    // "<data_attribute_name>-<asc|desc>", e.g.
    // "adminname-desc".
    ...
    // of the value for
    // the select tag option along with the ascending or descending suffix. It
    // should also be used as
    // the data attribute name in the HTML that renders the question. The value of
    // the data attribute
    // should contain the actual data to sort on.
    private final String dataAttribute;
    // Display string for the select tag option for the option that sorts by this
    // attribute in
    // descending order. Not present if this attribute should not have a descending
    // option.
    private final Optional<String> displayStringDescending;
    // Display string for the select tag option for the option that sorts by this
    // attribute in
    // ascending order. Not present if this attribute should not have an ascending
    // option.
    private final Optional<String> displayStringAscending;

    private TrustedIntermediarySortOption(
    ...
    public ImmutableList<SelectWithLabel.OptionValue> getSelectOptions() {
        ImmutableList.Builder<SelectWithLabel.OptionValue> tISortOptionsBuilder = ImmutableList
                .builder();
        if (displayStringAscending.isPresent()) {
            tISortOptionsBuilder.add(
        ...
        return tISortOptionsBuilder.build();
    }
}