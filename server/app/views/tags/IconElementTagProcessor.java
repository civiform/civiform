...
public final class IconElementTagProcessor extends AbstractElementTagProcessor {
  ...
  private void verifyIconExists(Path assetPath) {
    // In dev/test try to verify the physical file exists
    if (!environment.isProd()) {
      File file =
          environment.getFile(Paths.get(ASSETS_RELATIVE_PATH).resolve(assetPath).toString());

      if (!file.exists()) {
        throw new RuntimeException(String.format("Can't find icon file at path: %s", assetPath));
      }
    }

    // In all environments check if the assetFinder path ends with the relative file
    // path. When it does that means the autogenerated file hash that is added to
    // the url path isn't there which indicates the assetFinder couldn't find the
    // file so it is falling back to the plain path which doesn't get served by
    // the webserver.
    String assetFinderPath = assetsFinder.path(assetPath.toString());

    if (assetFinderPath.endsWith(assetPath.toString())) {
      throw new RuntimeException(
          String.format(
              "Can't find icon file at: %s, assetFinderPath: %s", assetPath, assetFinderPath));
    }
  }
