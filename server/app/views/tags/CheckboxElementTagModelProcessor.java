...
import views.tags.settings.CheckboxFormSettings;
import views.tags.settings.FormSettings;
import views.tags.settings.ItemFormSettings;

/**
 * Custom Thymeleaf element that will render into checkbox(es) with all appropriate HTML to be
 * structured correctly in terms of labels, validation messages, etc.
 *
 * <p>Sample Thymeleaf:
 *
 * <pre>{@code <cf:checkbox name="notifications" label="Notifications">
 ...
 * }</pre>
 *
 * @see <a href="http://localhost:9000/dev/component/catalog/checkbox">Checkbox Component -
 *     Component Catalog</a>
 */
public final class CheckboxElementTagModelProcessor extends AbstractElementModelProcessor {

  private static final String TAG_NAME = "checkbox";
  private static final int PRECEDENCE = 900;

  public CheckboxElementTagModelProcessor(final String dialectPrefix) {
    super(
        TemplateMode.HTML,
        dialectPrefix, // Prefix for <cf:checkbox>
        TAG_NAME,
  ...
  protected void doProcess(
    ...
    CheckboxFormSettings elementSettings =
        CheckboxFormSettings.builder()
    ...
    String checkboxGroupCssClasses =
        BetterStringJoiner.withSpaceDelimiter()
            .add("checkbox-group")
            .addIf(isNotBlank(elementSettings.columns().value()), elementSettings.columnCssClass())
            .toString();

    var checkboxGroupAttrs = new BetterLinkedHashMap<String, String>();
    checkboxGroupAttrs.put("class", checkboxGroupCssClasses);

    // Add checkbox-group div
    targetModel.add(modelFactory.createOpenElementTag("div", checkboxGroupAttrs, null, false));

    // Process each item (checkbox)
    for (var itemSettings : itemSettingsList) {
      var checkboxDivAttrs = new BetterLinkedHashMap<String, String>();
      checkboxDivAttrs.put("class", "usa-checkbox");
      checkboxDivAttrs.putIf(
          isNotBlank(itemSettings.thEach().value()), "th:each", itemSettings.thEach().value());

      // Add usa-checkbox div
      targetModel.add(modelFactory.createOpenElementTag("div", checkboxDivAttrs, null, false));

      addCheckboxInputElement(itemSettings, elementSettings, targetModel, modelFactory);
      addCheckboxLabelElement(itemSettings, targetModel, modelFactory);
  ...
  private static void addCheckboxInputElement(
    ...
    String inputCssClasses =
        BetterStringJoiner.withSpaceDelimiter()
            .add("usa-checkbox__input")
            .addIf(elementSettings.tiled().valueAsBoolean(), "usa-checkbox__input--tile")
            .addIf(!elementSettings.isValid().valueAsBoolean(), "usa-checkbox__input--error")
            .toString();
    ...
    // Add checkbox input (self-closing)
    targetModel.add(
        modelFactory.createStandaloneElementTag("input", inputAttrs, null, false, true));
  }

  private static void addCheckboxLabelElement(
    ...
    labelAttrs.put("class", "usa-checkbox__label");
  ...
  private static void addDescriptionSpanElement(
    ...
    var descAttrs = Map.of("class", "usa-checkbox__label-description");
