...
public final class ProgramApplicationsApiController extends CiviFormApiController {
   ...
   *
   * @param paginationToken - optional token containing pagination state from previous request
   * @param fromDateParam - optional inclusive start date for filtering submissions
   * @param toDateParam - optional exclusive end date for filtering submissions
   * @param revisionStateParam - optional revision state filter
   * @return configured filter object for application queries
   */
   ...
   * @return pagination specification for sequential row ID-based pagination
   */
   ...
   * requests.
   *
   * @param paginationResult - current page results containing applications and pagination metadata
   * @param programSlug - unique identifier for the program being queried
   * @param pageSize - number of records to return per page
   * @param timeFilter - date range filter for application submissions
   * @param lifecycleStages - lifecycle stages filter
   * @return Optional containing the pagination token if more pages exist, empty otherwise
   */
  private Optional<ApiPaginationTokenPayload> getNextPageToken(
    ...
    // Store lifecycle stages as comma-separated string
    if (!lifecycleStages.isEmpty()) {
      String stagesString =
          lifecycleStages.stream().map(LifecycleStage::name).collect(Collectors.joining(","));
      requestSpec.put(REVISION_STATE_PARAM_NAME, stagesString);
    }
   ...
   *
   * @param apiPaginationTokenPayload Optional pagination token containing previous request context
   * @param revisionStateParam Optional revision state parameter from the current request
   * @return Optional LifecycleStage resolved from the inputs, or empty if no revision state
   *     specified
   * @throws BadApiRequestException if pagination token and query parameter specify different values
   */
  private ImmutableList<LifecycleStage> resolveRevisionStateParam(
      Optional<ApiPaginationTokenPayload> apiPaginationTokenPayload,
      Optional<String> revisionStateParam) {
    if (revisionStateParam.isEmpty()) {
    ...
    ImmutableList<LifecycleStage> tokenLifecycleStages =
        apiPaginationTokenPayload
            .map(
                (token) ->
                    extractLifecycleStagesFromPaginationToken(token, revisionStateParam.get()))
            .orElse(ImmutableList.of());
    ImmutableList<LifecycleStage> queryLifecycleStages = getLifecycleStages(revisionStateParam);
   ...
   * @param paramName The parameter name to look for in the token's request spec
  ...
  private ImmutableList<LifecycleStage> extractLifecycleStagesFromPaginationToken(
    ...
    String stagesString = requestSpec.get(REVISION_STATE_PARAM_NAME);
    return Arrays.stream(stagesString.split(","))
        .map(String::trim)
        .map(LifecycleStage::valueOf)
        .collect(ImmutableList.toImmutableList());
  }

  /**
   * Converts a revision state parameter string to the corresponding LifecycleStage enum, or to the
   * default values if none is provided,
   *
   * @param revisionStateParam Optional string representation of revision state
   * @return Corresponding LifecycleStage enum values
   * @throws BadApiRequestException if the parameter doesn't match any valid RevisionState
   */
  private ImmutableList<LifecycleStage> getLifecycleStages(Optional<String> revisionStateParam) {
    // Default to all submitted application if no revision state is provided
    if (revisionStateParam.isEmpty()) {
      return ImmutableList.of(LifecycleStage.ACTIVE, LifecycleStage.OBSOLETE);
    }
    try {
      RevisionState state = RevisionState.valueOf(revisionStateParam.get());
      return ImmutableList.of(
          state == RevisionState.CURRENT ? LifecycleStage.ACTIVE : LifecycleStage.OBSOLETE);
    } catch (IllegalArgumentException e) {
      throw new BadApiRequestException(
          "Invalid revision state parameter: " + revisionStateParam.get());
    }
  }
}
