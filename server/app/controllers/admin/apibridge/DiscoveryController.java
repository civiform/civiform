...
import views.admin.apibridge.ErrorPartialView;
import views.admin.apibridge.ErrorPartialViewModel;
...
public class DiscoveryController extends Controller {
  ...
  private final ErrorPartialView errorPartialView;
  ...
  public DiscoveryController(
      ...
      ErrorPartialView errorPartialView) {
    ...
    this.errorPartialView = checkNotNull(errorPartialView);
  }
  ...
  public CompletionStage<Result> hxDiscoveryPopulate(Http.Request request) {
    ...
    if (environment.isProd() && !command.getHostUrl().startsWith("https://")) {
      return CompletableFuture.completedFuture(
          createErrorView(request, "URL must start with HTTPS"));
    }
        ...
        .exceptionallyAsync(
            ex -> {
              ...
              return createErrorView(
                  request, "An error occurred. Verify the URL is correct and try again.");
            },
            classLoaderExecutionContext.current());
  }
  ...
  public CompletionStage<Result> hxAdd(Http.Request request) {
    ...
    return apiBridgeService
        ...
        .thenComposeAsync(
            ...
            classLoaderExecutionContext.current())
        .thenApplyAsync(
            v -> ok("Saved successfully").as(Http.MimeTypes.HTML),
            classLoaderExecutionContext.current())
        .exceptionally(
            ex -> {
              log.error("Failed to add endpoint", ex);
              return createErrorView(request, "An error occurred tryping to add endpoint");
            });
  }

  private Result createErrorView(Http.Request request, String... errors) {
    var errorViewModel =
        ErrorPartialViewModel.builder().errors(ImmutableList.copyOf(errors)).build();

    return ok(errorPartialView.render(request, errorViewModel)).as(Http.MimeTypes.HTML);
  }
}
