...
public class TrustedIntermediaryDashboardView extends BaseHtmlView {
    ...
    public Content render(TrustedIntermediaryGroup tiGroup, Http.Request request, Messages messages) {
        return layout.render(request, messages,
                TagCreator.main(body(
                renderHeader(tiGroup.getName()),
                h2(tiGroup.getDescription()).withClasses(Styles.ML_2),
                        hr(),
                        renderHeader("Group Members"),

                        renderTIMembersTable(tiGroup),
                        hr(),
                        renderHeader("Managed Applicants"),
                        renderTIApplicantsTable(tiGroup),
                        hr(),
                        renderAddNewButton(tiGroup, request))
                ).withClasses(
                                        Styles.PX_2, Styles.MAX_W_SCREEN_XL, Styles.MX_AUTO
                )
        );
    }
