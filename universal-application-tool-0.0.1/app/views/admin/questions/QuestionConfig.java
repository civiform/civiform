...
public class QuestionConfig {
  ...
  private final static String HEADER_CLASSES = StyleUtils.joinStyles(
  Styles.BG_TRANSPARENT, Styles.TEXT_GRAY_600, Styles.BLOCK, Styles.FONT_BOLD,
  Styles.TEXT_SM, Styles._MT_1, Styles.PB_0, Styles.MB_0, Styles.MX_2, Styles.UPPERCASE);

  private final static String INNER_DIV_CLASSES = StyleUtils.joinStyles(
  Styles.BORDER, Styles.BG_GRAY_100,
  Styles.PT_4, Styles.M_4);

  private final static String OUTER_DIV_CLASSES = StyleUtils.joinStyles(
  Styles.W_FULL, Styles.PT_0, Styles._MT_4);
  ...
  public static ContainerTag buildQuestionConfig(QuestionType type) {
    ...
    switch (type) {
      case ADDRESS:
        return config.setId("address-question-config")
            .addAddressQuestionConfig().getContainer();
      case TEXT:
        return config.setId("text-question-config")
            .addTextQuestionConfig().getContainer();
    ...
    return div()
        ...
        .with(
            div()
                ...
                .with(content.withClasses(INNER_DIV_CLASSES))
        );
  }

  private static ContainerTag headerLabel(String text) {
    return label()
        .withClasses(HEADER_CLASSES)
        .withText(text);
  }
