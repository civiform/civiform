 ...
 *
 * When there is no repeated entity in scope, this should be an {@link #empty()} enumerator context.
 */
@AutoValue
public abstract class EnumeratorContext {
  ...
  public static EnumeratorContext create(RepeatedEntity repeatedEntity, ImmutableList<RepeatedEntity> ancestors, ImmutableList<RepeatedEntity> siblings, Path contextualizedPath) {
    return new AutoValue_EnumeratorContext(Optional.of(repeatedEntity), ancestors, siblings, contextualizedPath);
  }

  /** The enumerator context when there is no repeated entity in scope. */
  public static EnumeratorContext empty() {
    return new AutoValue_EnumeratorContext(Optional.empty(), ImmutableList.of(), ImmutableList.of(), ApplicantData.APPLICANT_PATH);
  }

  /** True if there is no repeated entity in scope. */
  public boolean isEmpty() { return repeatedEntity().isEmpty(); }

  /** Returns a new enumerator context for a nested repeated entity. */
  public EnumeratorContext append(RepeatedEntity repeatedEntity, ImmutableList<RepeatedEntity> siblings) {
    ImmutableList<RepeatedEntity> updatedAncestors = ImmutableList.<RepeatedEntity>builder()
        .addAll(ancestors())
        .add(repeatedEntity)
        .build();
    Path updatedPath = contextualizedPath()
