...
class ToastController {
  ...
  static warningSvgPath = "M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742"
  ...
  constructor() {
    ...
    this.toastContainer.classList.add("absolute", "hidden", "left-1/2", "top-0",
  ...
  showToastMessage(message: ToastMessage) {
    ...
    toastMessage.classList.add("bg-opacity-90", "duration-300", "flex", "flex-row",
      "max-w-md", "px-2", "py-2", "my-3", "relative", "rounded-sm", "shadow-lg",
      "transition-opacity", "transform", "text-gray-700");
    ...
    if (message.canDismiss) {
      ...
      dismissButton.classList.add("absolute", "font-bold", "pl-6", "opacity-40", "right-4", "top-2",
    ...
    if (toastContainer) {
      ...
      if (message.duration > 0) {
        setTimeout(ToastController.dismissToast, message.duration, message.id, /* dismissClicked = */ false);
      }
    }
  }
