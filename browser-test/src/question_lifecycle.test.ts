...
describe('normal question lifecycle', () => {
  ...
  it('shows primary applicant info toggles/alerts correctly when creating a new question, and tag is persisted', async () => {
    ...
    await adminQuestions.expectPrimaryApplicantInfoUniversalAlertVisible(true)
    ...
    await adminQuestions.clickUniversalToggle()
    await adminQuestions.expectPrimaryApplicantInfoUniversalAlertVisible(false)
    await adminQuestions.expectPrimaryApplicantInfoToggleVisible(
      nameField,
      true,
    )
  ...
  it('shows the alert when a different question has the primary applicant info tag', async () => {
    ...
    await adminQuestions.expectPrimaryApplicantInfoAlreadySetAlertVisible(true)
    await validateScreenshot(page, 'primary-applicant-info-already-set')

    await adminQuestions.gotoQuestionEditPage(paiNameQuestion)
    await adminQuestions.expectPrimaryApplicantInfoAlreadySetAlertVisible(false)
  })
})
