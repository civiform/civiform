...
test.describe('Program list page.', () => {
  ...
  test('shows information about universal questions when the flag is enabled and at least one universal question is set', async ({
    ...
    await adminPrograms.gotoAdminProgramsPage()
    await expect( page.innerText('.cf-admin-program-card')).not.toContain(
      'universal questions',
    )
    ...
    await adminPrograms.gotoAdminProgramsPage()
    await expect( page.innerText('.cf-admin-program-card')).toContain(
      'universal questions',
    )
    await validateScreenshot(
      page,
  ...
  test('publishing a single program shows a modal with conditional warning about universal questions', async ({
    ...
    await expect( page.innerText(publishProgramOneModal)).toContain(
    ...
    await expect( page.innerText(publishProgramOneModal)).not.toContain(
