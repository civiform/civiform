...
describe('applicant navigation flow', () => {
  ...
  describe('navigation with four blocks', () => {
    ...
    beforeAll(async () => {
      ...
      await adminQuestions.addAddressQuestion({questionName: 'nav-address-q'});
      await adminQuestions.addRadioButtonQuestion({questionName: 'nav-radio-q', options: ['one', 'two', 'three']});
      await adminQuestions.addStaticQuestion({questionName: 'nav-static-q'});

      await adminPrograms.addProgram(programName);
      await adminPrograms.editProgramBlock(programName, 'first description', ['nav-date-q', 'nav-email-q']);
      await adminPrograms.addProgramBlock(programName, 'second description', ['nav-static-q']);
      await adminPrograms.addProgramBlock(programName, 'third description', ['nav-address-q']);
      await adminPrograms.addProgramBlock(programName, 'fourth description', ['nav-radio-q']);
    ...
    it('clicking previous on later blocks goes to previous blocks', async () => {
      ...
      await applicantQuestions.clickPrevious();
      await applicantQuestions.checkAddressQuestionValue('1234 St', 'Unit B', 'Sim', 'Ames', '54321');
