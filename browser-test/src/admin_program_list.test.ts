...
describe('Program list page.', () => {
  ...
  it('publish single program shows a modal with conditional warning about universal questions', async() => {
    ...
    await adminQuestions.addNameQuestion({questionName: nameQuestion, universal: false})
    ...
    expect(await page.innerText('#program-one-publish-modal')).toContain('Are you sure you want to publish program one and all of its draft questions?')
    // Warning should not show because there are no universal questions
    expect(await page.innerText('#program-one-publish-modal')).not.toContain('Warning: This program does not use all recommended universal questions.')
    ...
    await adminQuestions.addTextQuestion({questionName: textQuestion, universal: true})
    await adminPrograms.gotoAdminProgramsPage()
    await page.click('#program-one-publish-modal-button')
    // Warning should show because there is a universal question that is not used in this program
    expect(await page.innerText('#program-one-publish-modal')).toContain('Warning: This program does not use all recommended universal questions.')
