...
class FilePlacementReporter implements Reporter {
  ...
  copyActualImageToUpdatedSnapshotsFolder(actual: Attachment) {
    // Copy actual file to updated_snapshots
    const updatedSnapshotFile = path.join(
      './updated_snapshots',
      actual.name.replace('-actual.png', '-received.png'),
    )
    fs.mkdirSync(path.dirname(updatedSnapshotFile), {recursive: true})
    fs.copyFileSync(actual.path, updatedSnapshotFile)
  }
